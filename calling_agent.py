...
async def entrypoint(ctx: JobContext) -> None:
    ...
    system_prompt = (
        ...
        f"     propose that specific time to the other person, and ONLY if they give a clear yes, "
        f"     call create_calendar_event to book it. Do not ask {user_name} — just do it. "
        f"     Vague sounds like 'hmm', 'okay', 'sure' without a time are NOT confirmation — ask them to confirm the time explicitly. "
        f"     After booking, explicitly confirm: e.g. 'Done, I've added it to {user_name}'s calendar — see you then!'\n"
        f"   - Use your best judgement for similar small logistics.\n"
        f"4. STRICT TOOL ORDER — you must follow this sequence exactly, one step per turn:\n"
        f"   a. Call create_calendar_event (if needed) and wait for its result.\n"
        f"   b. Speak the confirmation and farewell in one reply.\n"
        f"   c. Only after the farewell is spoken, call end_call — NEVER in the same turn as create_calendar_event.\n"
        f"   NEVER call end_call and create_calendar_event together in the same response.\n"
        f"   NEVER call end_call before create_calendar_event has returned a result.\n"
